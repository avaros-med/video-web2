...
mockUseVideoContext.mockImplementation(() => ({
    setIsBackgroundSelectionOpen: mockSetIsBackgroundSelectionOpen,
}))
...
describe('the ToggleChatButton component', () => {
    ...
    it('should show an indicator when there are unread messages', () => {
        ...
        const notificationCircle = wrapper.findWhere(node =>
            node.prop('className')?.includes('circle')
        )
        expect(notificationCircle.prop('className')).toContain(
            'hasUnreadMessages'
        )
    })
    ...
    it('should not show an indicator when there are no unread messages', () => {
        ...
        expect(notificationCircle.prop('className')).not.toContain(
            'hasUnreadMessages'
        )
    })
    ...
    it(`should add the 'animate' class for ${ANIMATION_DURATION}ms when a new message is received when the chat window is closed`, () => {
        ...
        const wrapper = mount(<ToggleChatButton />)
        let notificationRing = wrapper.findWhere(node =>
            node.prop('className')?.includes('ring')
        )
        ...
        notificationRing = wrapper.findWhere(node =>
            node.prop('className')?.includes('ring')
        )
